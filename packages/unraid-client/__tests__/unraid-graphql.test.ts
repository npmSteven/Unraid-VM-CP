...
import { initGraphQLClient, startVM, stopVM, forceStopVM, rebootVM, pauseVM, resumeVM, _resetForTests } from '../src/unraid-graphql.js';
...
describe('unraid-graphql', () => {
  beforeEach(() => {
    ...
    mockFetch.mockReset();
    _resetForTests();
    initGraphQLClient(() => 'test-cookie', 'http://test-unraid:80');
  });
  ...
  describe('no cookie', () => {
    it('throws when cookie getter returns empty', async () => {
      _resetForTests();
      initGraphQLClient(() => '', 'http://test-unraid:80');
